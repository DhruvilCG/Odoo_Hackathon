...
export default function Purchases() {
  ...
  useEffect(() => {
    if (user) setPurchases(api.listPurchases(user.id));
  }, [user]);
    ...
    <div className="bg-gray-100">
      ...
      <section className="bg-gradient-to-b from-purple-900 to-purple-700 text-white p-6 sm:p-12 flex justify-center items-center">
        <div className="bg-white p-6 sm:p-8 rounded-xl shadow-lg w-full max-w-4xl">
          <h2 className="text-3xl sm:text-4xl font-bold mb-6 text-purple-600 text-center">Previous Purchases</h2>
          <div className="space-y-6">
            {purchases.map((p) => (
              <div key={p.id} className="bg-white p-4 sm:p-6 rounded-xl shadow hover:shadow-lg transition">
                <div className="text-gray-500 text-sm sm:text-base mb-3">{new Date(p.date).toLocaleString()}</div>
                <div className="space-y-2">
                  {p.items.map((it) => (
                    <div key={it.id} className="flex items-center gap-4 bg-gray-50 p-2 sm:p-3 rounded-lg">
                      <div className="w-16 sm:w-20 h-16 sm:h-20 bg-gray-200 flex items-center justify-center rounded-lg text-gray-500 font-semibold">
                        Img
                      </div>
                      <div>
